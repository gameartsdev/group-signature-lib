...
public:
    ...
    explicit LinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>> &config) : pconfig_interface(config), pbc_param_inited(false) {}
                                         ...
                                         int &ret_code)
    {
        ...
        int ret = pbc_param_init_set_str(param_ptr, pbc_param_str.c_str());
        if (ret)
            ret_code = RetCode::GroupSigStatusCode::BAD_PARAM;
        else
...
public:
    ...
    ALinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>> &config,
                string q_key = "q_bits_len", string r_key = "r_bits_len") : LinearPair(config),
...
public:
    ...
    DLinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>> &config,
                int D_value = DEFAULT_D, string D_key = "D") : LinearPair(config), D(D_value)
    {
        if (pconfig_interface)
        {
...
public:
    ...
    ELinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>> &config,
                ...
                string q_key = "q_bits_len") : LinearPair(config),
...
{
public:
    GLinearPair(shared_ptr<ConfigParserInterface<JsonConfigParser>> &config) : LinearPair(config)
    {
    }
